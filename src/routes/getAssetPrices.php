<?php
$app->post('/api/SteamWeb/getAssetPrices', function ($request, $response, $args) {
    $settings = $this->settings;

    //checking properly formed json
    $checkRequest = $this->validation;
    $validateRes = $checkRequest->validate($request, ['appId', 'apiKey']);
    ...
    $query_str = $settings['api_url'] . "ISteamEconomy/GetAssetPrices/" . $version . '/';
    ...
    if (isset($post_data['args']['language']) && strlen($post_data['args']['language']) > 0) {
        $body['language'] = $post_data['args']['language'];
    }
    if (isset($post_data['args']['currency']) && strlen($post_data['args']['currency']) > 0) {
